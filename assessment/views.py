...
#@permission_required('assessment.can_view_assessment', raise_exception=PermissionDenied)
...
@permission_required('assessment.add_assessment', raise_exception=PermissionDenied)
...
@permission_required('assessment.change_assessment', raise_exception=PermissionDenied)
def updateresult(request, ass_id):
    ...
    if request.method == "POST":
        ...
        # to save the marks in database
            ...
            for s in students:
                c[s.user_id] = {}
                c[s.user_id]["name"]=s.user.username
                c[s.user_id]["marks"] = {}
            ...
            max_marks.reverse()
            return render(request, 'assessment/updateresult.html', {'form': form, 'students': c, 'questions': q,'assessment':assessment,'marks':max_marks})

#@permission_required('assessment.add_asseessment_question', raise_exception=PermissionDenied)
...
@permission_required('assessment.can_view_result_student', raise_exception=PermissionDenied)
...
@permission_required('assessment.can_view_result_faculty', raise_exception=PermissionDenied)
